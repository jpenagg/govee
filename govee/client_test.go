...
func TestUnmarshalSampleResponse(t *testing.T) {
	...
	dev := resp.Data[0]
	if dev.DeviceName != "52 York" || dev.SKU != "H706A" {
		t.Errorf("Wrong device info: %+v", dev)
	}

	segCap := findCapability(dev.Capabilities, "segmentedColorRgb")
	if segCap == nil {
		t.Fatal("segmentedColorRgb capability missing")
	}

	segField := findField(segCap.Parameters.Fields, "segment")
	if segField == nil || segField.ElementRange == nil {
		t.Fatal("Failed to parse segment field")
	}
...
func findCapability(caps []Capability, instance string) *Capability {
	for _, c := range caps {
